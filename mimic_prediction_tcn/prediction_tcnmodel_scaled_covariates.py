...
for parameter in parameters:
    ...
    for window_idx in range(n_windows):
        ...
        pred_scalers_high_f = open(f'./data/{approach}/{n_chunks}_chunks/{style}/{parameter}/{endogenous_input}/'
                                   f'03_pred_scalers_high_window{window_idx}.pickle', 'wb')
        pickle.dump(pred_scalers_high, pred_scalers_high_f, protocol=pickle.HIGHEST_PROTOCOL)
        pred_scalers_high_f.close()

        pred_scalers_low_f = open(f'./data/{approach}/{n_chunks}_chunks/{style}/{parameter}/{endogenous_input}/'
                                  f'03_pred_scalers_low_window{window_idx}.pickle', 'wb')
    ...
    confusion_matrix_models = confusion_matrix_models.append({
        # T = TCNModel, model_numbers = {01, ..., 04} and H = High
        'ID': f'{parameter.upper()}_T_04_H',
        'PARAMETER': parameter.upper(),
        'RUNTIME': runtime,
        'MODEL': 'TCN',
        'SCALED': True,
    ...
    confusion_matrix_models = confusion_matrix_models.append({
        # T = TCNModel, model_numbers = {01, ..., 04} and L = Low
        'ID': f'{parameter.upper()}_T_04_L',
        'PARAMETER': parameter.upper(),
        'RUNTIME': runtime,
        'MODEL': 'TCN',
        'SCALED': True,
