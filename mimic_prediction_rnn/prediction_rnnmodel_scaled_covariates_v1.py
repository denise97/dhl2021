"""
    PREDICTION WITH ALL RNN MODELS, ALL PARAMETERS, AND MAX AND MIN RESAMPLED CHUNKS WHICH ARE Z-SCALED
    ...
    for parameter in parameters:
        ...
        for window_idx in range(n_windows):
            ...
            param_model_high_f = open(f'./data/{approach}/{n_chunks}_chunks/{style}/{model_type}/{parameter}/'
                                      f'{endogenous_input}/04_pre-trained_model_high_scaled_v1_window{window_idx}.pickle',
                                      'wb')
            ...
            param_model_low_f = open(f'./data/{approach}/{n_chunks}_chunks/{style}/{model_type}/{parameter}/'
                                     f'{endogenous_input}/04_pre-trained_model_low_scaled_v1_window{window_idx}.pickle',
                                     'wb')
            ...
            for chunk_id in pred_series_endo_high.keys():
                ...
                model_original_f = open(f'./data/{approach}/{n_chunks}_chunks/{style}/{model_type}/{parameter}/'
                                        f'{endogenous_input}/04_pre-trained_model_high_scaled_v1_window{window_idx}'
                                        f'.pickle', 'rb')
                ...
                for iteration in range(len(pred_series_endo_high[chunk_id]) - input_length):
                    ...
                    current_pred_high = z_retransform(current_pred_high, parameter, window_idx)
                ...
                final_pred_high_f = open(f'./data/{approach}/{n_chunks}_chunks/{style}/{model_type}/{parameter}/'
                                         f'{endogenous_input}/05_prediction_{chunk_id}_high_scaled_v1_window{window_idx}'
                                         f'.pickle', 'wb')
                ...
                for iteration in range(len(pred_series_endo_low[chunk_id]) - input_length):
                    ...
                    # Rescale predicted measurement (returned as DataFrame)
                    current_pred_low = z_retransform(current_pred_low, parameter, window_idx)
                ...
                final_pred_low_f = open(f'./data/{approach}/{n_chunks}_chunks/{style}/{model_type}/{parameter}/'
                                        f'{endogenous_input}/05_prediction_{chunk_id}_low_scaled_v1_window{window_idx}'
                                        f'.pickle', 'wb')
